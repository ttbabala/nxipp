...
<div id="dcMain">
   <!-- 当前位置 -->
    ...
    <div class="mainBox" style="height:auto!important;height:550px;min-height:550px;">
        <h3><a href="<?php echo url('user/adduser'); ?>" class="actionBtn add">添加用户</a><?php echo $columnName; ?></h3>
        <div id="list">
                <table width="100%" border="0" cellpadding="8" cellspacing="0" class="tableBasic">
                 ...
                 <tr>
                      ...
                      <td align="left"><?php echo $user['uname']; ?></td>
                      <td align="left"><?php echo $user['uemail']; ?></td>
                      <td align="left"><?php echo $user['create_time']; ?></td>
                      <td align="left"><?php echo $user['last_logintime']; ?></td>
                      <td align="left"><?php echo $user['last_ip']; ?></td>
                      <td align="left">
                          <?php if(is_array($partData) || $partData instanceof \think\Collection): $i = 0; $__LIST__ = $partData;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;if($user['authorization'] == $vo['id']): ?>
                                   <?php echo $vo['title']; endif; endforeach; endif; else: echo "" ;endif; ?>
                      </td>
                      <td align="left">
                          <?php echo !empty($user['status']=1)?'正常' : '锁定'; ?>
                      </td>
                      <td align="center">
                          <a href="<?php echo url('User/editUser',['id' => $user['id']]); ?>" class="tab_a_link edit_btn">修改</a>
                          <a class="tab_a_link del_btn btn-danger-a" href="javascript:;" userid="<?php echo $user['id']; ?>"><i class="fa fa-share-square-o">删除</i></a>
                      </td>
                 </tr>
            ...
            $(function(){
                $('.btn-danger-a').on('click',function(){
                    ...
                    if (del) {
                        ...
                        $.post('<?php echo url("User/delUser"); ?>',{userid : userid}, function(data){
                           if (data.status) {
                                layer.msg(data.msg, {icon: 1,time: 1500},function(){
                                    window.location.reload();
                                });
