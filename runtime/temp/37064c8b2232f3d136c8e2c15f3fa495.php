...
<div id="dcMain">
   <!-- 当前位置 -->
    ...
    <div class="mainBox" style="height:auto!important;height:550px;min-height:550px;">
        <h3><a href="<?php echo url('Notice/addNotice'); ?>" class="actionBtn add">添加公告</a>公告列表</h3>
        <div id="list">
                <table width="100%" border="0" cellpadding="8" cellspacing="0" class="tableBasic">
                 ...
                 <?php if(is_array($noticelist) || $noticelist instanceof \think\Collection): $i = 0; $__LIST__ = $noticelist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                 <tr>
                      <td align="left"><?php echo $vo['id']; ?></td>
                      <td align="left"><?php echo $vo['title']; ?></td>
                      <td align="left"><?php echo $vo['excerpt']; ?></td>
                      <td align="left"><?php echo $vo['createtime']; ?></td>
                      <td align="left"><?php echo $vo['updatetime']; ?></td>
                      <td align="left"><?php echo !empty($vo['comments']=1)?'开启' : '关闭'; ?></td>
                      <td align="left"><?php echo !empty($vo['isshow']=1)?'显示' : '隐藏'; ?></td>
                      <td align="left"><?php echo $vo['author']; ?></td>
                      <td align="center">
                          <a href="<?php echo url('Notice/editNotice',['id' => $vo['id']]); ?>" class="tab_a_link edit_btn">编辑</a>
        ...
        <script type="text/javascript">
            $(function(){
                $('.btn-danger-a').on('click',function(){
                    ...
                    if (del) {
                        var id =$(this).attr('id');
                        $.post('<?php echo url("Notice/delNotice"); ?>',{id : id}, function(data){
                           if (data.status) {
                                layer.msg(data.msg, {icon: 1,time: 1500},function(){
                                    window.location.reload();
                                });
