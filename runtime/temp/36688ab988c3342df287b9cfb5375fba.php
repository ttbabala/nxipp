...
<div id="dcMain">
     <div id="urHere">管理中心<b>></b><strong>规则管理</strong> </div>
     <div class="mainBox" style="height:auto!important;height:550px;min-height:550px;">
         <h3><a href="<?php echo url('Rule/addRules'); ?>" class="actionBtn add">批量添加规则</a><a href="<?php echo url('Rule/addRule'); ?>" class="actionBtn add">逐条添加规则</a>规则管理</h3>
          <div id="list">
                <table width="100%" border="0" cellpadding="8" cellspacing="0" class="tableBasic">
                 <tr>
                      <th width="20" align="left">规则ID</th>
                      <th width="50" align="left">规则名称</th>
                      <th width="100" align="left">应用规则</th>
                      <th width="20" align="left">规则状态</th>
                      <th width="80" align="center">自定义规则</th>
                      <th width="80" align="center">操作</th>
                 </tr>
                 <?php if(is_array($RulelistData) || $RulelistData instanceof \think\Collection): $i = 0; $__LIST__ = $RulelistData;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                    <tr>
                         <td align="left"><?php echo $vo['id']; ?></td>
                         <td align="left"><?php echo $vo['title']; ?></td>
                         <td align="left"><?php echo $vo['name']; ?></td>
                         <td align="left"><?php echo !empty($vo['status']=1)?'正常' : '禁用'; ?></td>
                         <td align="left"><?php echo $vo['condition']; ?></td>
                         <td align="center">
                             <a href="<?php echo url('Rule/editRule',['id' => $vo['id']]); ?>" class="tab_a_link edit_btn">编辑</a>
        ...
        <script type="text/javascript">   
            $(function(){
                $('.btn-danger-a').on('click',function(){
                    ...
                    if (del) {
                        var id =$(this).attr('id');
                        $.post('<?php echo url("Rule/delRule"); ?>',{id : id}, function(data){
                           if (data.status) {
                                layer.msg(data.msg, {icon: 1,time: 1500},function(){
                                    window.location.reload();
                                });
