...
<div id="dcMain">
   <!-- 当前位置 -->
    ...
    <div class="mainBox" style="height:auto!important;height:550px;min-height:550px;">
        <h3><a href="<?php echo url('Members/addMember'); ?>" class="actionBtn add">添加会员</a>会员列表</h3>
        <div class="filter">
            <form id="formfilter" action="<?php echo url('Members/index'); ?>" method="get">
                 <select name="cat_id">
        ...
        <div id="list">
            <form name="action" id="formaction" method="post" action="#">
                <table width="100%" border="0" cellpadding="8" cellspacing="0" class="tableBasic">
                 ...
                 <?php if(is_array($membersData) || $membersData instanceof \think\Collection): $i = 0; $__LIST__ = $membersData;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                 <tr>
                     <td align="center"><input type="checkbox" name="checkbox[]" value="<?php echo $vo['mid']; ?>" /></td>
                      <td align="left"><?php echo $vo['mid']; ?></td>
                      <td align="left"><img src="<?php echo $vo['headpic']; ?>" width="40px" height="40px" class="img"/></td>
                      <td align="left"><?php echo $vo['mname']; ?></td>
                      <td align="left"><?php echo $vo['email']; ?></td>
                      <td align="left"><?php echo $vo['register_time']; ?></td>
                      <td align="left"><?php echo $vo['gmt_time']; ?></td>
                      <td align="left"><?php echo $vo['gmt_ip']; ?></td>
                      <td align="left">
                          <?php echo !empty($vo['status']=1)?'正常' : '锁定'; ?>
                      </td>
                      <td align="center">
                          <a href="<?php echo url('Members/editMember',['mid' => $vo['mid']]); ?>" class="tab_a_link edit_btn">修改</a>
                          <a class="tab_a_link del_btn btn-danger-a" href="javascript:;" mid="<?php echo $vo['mid']; ?>"><i class="fa fa-share-square-o">删除</i></a>
                      </td>
                 </tr>
        ...
        <script type="text/javascript" src="http://localhost/nxipp/public/admin/plugins/jquery-1.8.3.min.js"></script>
        <script src="http://localhost/nxipp/public/admin/plugins/layer/layer.min.js"></script>
        <script type="text/javascript" src="http://localhost/nxipp/public/admin/plugins/laydate/laydate.js"></script>
        <script type="text/javascript">
            $(function(){
                $('.btn-danger-a').on('click',function(){
                    ...
                    if (del) {
                        ...
                        $.post('<?php echo url("Members/delMember"); ?>',{mid : mid}, function(data){
                           if (data.status) {
                                layer.msg(data.msg, {icon: 1,time: 1500},function(){
                                    window.location.reload();
                                });
         ...
         $(function(){
                $("#formfilter").submit(function(){
                    ...
                    $.post('<?php echo url("Members/index"); ?>',datas,function(data){
                       if (data.status) {
                           layer.msg(data.msg, {icon: 1,time: 1500},function(){
         ...
         $(function(){   //异步批量删除
                 $("#formaction").submit(function(){
                     ...
                     if( action === 'blacklist'){
                        ...
                        $.post('<?php echo url("Members/clMembers"); ?>',{post_str : post_str}, function(data){
                           if (data.status) {
