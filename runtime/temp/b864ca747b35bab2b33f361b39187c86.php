        ...
        <div id="list">
            <form name="fromZan" id="formZan" method="post" action="#">
                <table width="100%" border="0" cellpadding="8" cellspacing="0" class="tableBasic">
                 ...
                 <?php if(is_array($zanData) || $zanData instanceof \think\Collection): $k = 0; $__LIST__ = $zanData;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?>
                 <tr>
                      <td align="left"><?php echo $vo['id']; ?></td>
                      <td align="left"><?php echo $vo['article_title'][0]; ?></td>
                      <td align="left"><?php echo $vo['article_author'][0]; ?></td>
                      <td align="left"><?php echo $vo['article_date'][0]; ?></td>
                      <td align="left"><?php echo $vo['likes']; ?></td>
                      <td align="left"><?php echo $vo['like_percent']; ?>%</td>
                      <td align="left"><?php echo $vo['unlikes']; ?></td>
                      <td align="left"><?php echo $vo['unlike_percent']; ?>%</td>
                      <td align="center">
                          <a href="javascript:;" class="tab_a_link init_btn" value="<?php echo $vo['id']; ?>">初始化</a>
                      </td>
                 </tr>
            ...
            $(function(){   //单个初始化
                $('.init_btn').on('click',function(){
                    ...
                    if (init) {
                        ...
                        $.post('<?php echo url("Zan/initzan"); ?>',{datas: tiString}, function(data){
                           if (data.status) {
                                layer.msg(data.msg, {icon: 1,time: 1500},function(){
             ...
             $(function(){   //点赞全部初始化
                $('.initAll').on('click',function(){
                    ...
                    if (init) {
                        var type = 'all';
                        $.post('<?php echo url("Zan/initzan"); ?>',{type : type}, function(data){
                           if (data.status) {
                                layer.msg(data.msg, {icon: 1,time: 1500},function(){
                                      window.location.href = data.url;
                                  });
