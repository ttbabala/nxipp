...
class My_Widget5 extends WP_Widget{

	function My_Widget5()
	{
		$widget_ops = array('description' => '侧边栏的小图文，可以选择调用一个你建立好的分类，以小图模式显示(第一篇文章高亮为大图)，这个分类的文章必须设定特色图片');
		$control_ops = array('width' => 200, 'height' => 300);
		parent::WP_Widget(false,$name='高亮图片模块(WEB主题公园)',$widget_ops,$control_ops);  
...
<p>   
    <label  for="<?php echo $this->get_field_id('zhiding'); ?>">是否只显示置顶文章:</label><br />
             <select id="<?php echo $this->get_field_id('zhiding'); ?>" name="<?php echo $this->get_field_name('zhiding'); ?>" >
              <option value='2'<?php if ($zhiding == "2" ) {echo "selected='selected'";}?> >显示最新文章(推荐填写奇数)</option>
