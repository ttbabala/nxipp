...
class My_Widget3 extends WP_Widget {
	...
	function My_Widget3()
	{
		$widget_ops = array('description' => '侧边栏的活动图文模块，可以选择调用一个你建立好的分类，第一篇文章以小图、标题和描述模式显示，以后的文章以文字显示');
		$control_ops = array('width' => 200, 'height' => 300);
		parent::WP_Widget(false,$name='高亮图文模块(WEB主题公园)',$widget_ops,$control_ops);  
...
<p>   
    <label  for="<?php echo $this->get_field_id('zhiding'); ?>">是否只显示置顶文章:</label><br />
             <select id="<?php echo $this->get_field_id('zhiding'); ?>" name="<?php echo $this->get_field_name('zhiding'); ?>" >
              <option value='2'<?php if ($zhiding == "" ) {echo "selected='selected'";}?> >显示最新文章</option>
...
<?php  $ashu_i=0; while ( $query->have_posts() ) :$query->the_post(); $ashu_i++; ?>     
   ...
   <li class="firest">
        ...
        <a target="_blank" class="new_tu_img" href="<?php the_permalink() ?>"  title="<?php the_title(); ?>" >
         ...
         <?php 
		 if (has_post_thumbnail()) { the_post_thumbnail('news_smoll-sidbar-thumb' ,array('alt'	=>$tit, 'title'=> $tit ));} ?>
         </a> 
         
        <div class="shuom2"> 
        <a  title="<?php the_title() ?>"  target="_blank"class="s_tlt" href="<?php the_permalink() ?>"><?php echo mb_strimwidth(strip_tags(apply_filters('the_title', $tit)),  0,23,"..."); ?></a>
     
         <p title=" <?php echo mb_strimwidth(strip_tags(apply_filters('the_excerp',get_the_excerpt($id))),0,200,"..."); ?>">  
		 <?php echo mb_strimwidth(strip_tags(apply_filters('the_excerp',get_the_excerpt($id))),0,50,"..."); ?></p>
     ...
     <?php else : ?>
        ...
        <a target="_blank" href="<?php the_permalink() ?>"  title="<?php the_title(); ?>" >
          ...
          <?php echo mb_strimwidth(strip_tags(apply_filters('the_title', $tit)),  0,33,"..."); ?>
