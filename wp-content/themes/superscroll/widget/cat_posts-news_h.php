...
class My_Widget_h extends WP_Widget {

	function My_Widget_h()
	{
		$widget_ops = array('description' => '侧边栏的图文模块，可以选择调用一个你建立好的分类，以图文模式显示，显示大图和较小标题（显示长方形图片）');
		$control_ops = array('width' => 200, 'height' => 300);
		parent::WP_Widget(false,$name='长方形大图文分类模块(WEB主题公园)',$widget_ops,$control_ops);  
...
<?php   while ( $query->have_posts() ) :$query->the_post(); ?>     
   ...
   <li id="big">
        ...
        <a target="_blank" id="cfx3" class="new_tu_img" href="<?php the_permalink() ?>"  title="<?php the_title(); ?>" >
		 ...
		 if (has_post_thumbnail()) { the_post_thumbnail('defent-thumb' ,array('alt'	=>$tit, 'title'=> $tit ));} }?>
