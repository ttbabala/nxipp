...
//加载自定义小工具
include_once get_stylesheet_directory() .'/widgets/widget-newszan.php';
...
function _get_ssl2_avatar($avatar) {
    ...
    $avatar = get_bloginfo('template_directory') . '/img/website_top_logo.png';

	$my_avatar = "<img src='".$avatar."' width='50px' height='50px'/>";
...
add_filter( 'avatar_defaults', 'wenshuo_gravatar' );  

 function wenshuo_gravatar ($avatar_defaults) {  

    $my_avatar = get_bloginfo('template_directory') . '/img/website_top_logo.png';  
...
{    echo '<style type="text/css">body.login #login h1 a {background:url(http://xhxyps.duapp.com/wp-content/themes/dux/img/buttom_logo.gif) no-repeat 0 0 transparent;height: 31px;width: 155px;padding:0;margin:0 auto 1em;}</style>';}add_action("login_head", "nowspark_login_head");//modify the background image
